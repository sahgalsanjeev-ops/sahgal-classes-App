...
const TestAttemptPage = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setTestTitle(row.title);
      setQuestions(qs);
      const mins = Math.max(1, Number(row.duration_minutes) || 30);
      setTimeLeft(mins * 60);
    ...
    <div className="min-h-screen bg-background flex flex-col pb-4">
      ...
      <div className="flex-1 px-4 py-5">
        ...
        >
          ...
          <RadioGroup
            ...
            onValueChange={(val) => setAnswers((prev) => ({ ...prev, [q.id]: parseInt(val, 10) }))}
            className="space-y-3"
          >
            {q.options.map((opt, i) => (
              <label
              ...
              >
                <RadioGroupItem value={String(i)} id={`${q.id}-${i}`} className="mt-1 shrink-0" />
      ...
      <div className="p-4 bg-card border-t border-border flex items-center gap-3 mt-auto">
        ...
        {current < questions.length - 1 ? (
          <Button type="button" onClick={() => setCurrent((c) => c + 1)} className="h-12 rounded-xl flex-1">
            Next <ChevronRight size={18} />
          </Button>
        ) : (
          <Button type="button" onClick={goToResult} className="h-12 rounded-xl flex-1 bg-destructive hover:bg-destructive/90">
